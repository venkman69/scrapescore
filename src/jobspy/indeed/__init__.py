...
class Indeed(Scraper):
    def __init__(
        ...
        **kwargs,
        ...
        """
        super().__init__(Site.INDEED, proxies=proxies, ca_cert=ca_cert, user_agent=user_agent, **kwargs)
    ...
    def _process_job(self, job: dict) -> JobPost | None:
        ...
        job_url = job["recruit"].get("viewJobUrl") if job.get("recruit") else f"{self.base_url}/viewjob?jk={job['key']}"
        ...
        logger.info(f"Fetched details for job: {job['key']}, Title: {job['title']}, Description length: {len(description)} chars")
        return JobPost(
            id=f"in-{job['key']}",
            title=job["title"],
