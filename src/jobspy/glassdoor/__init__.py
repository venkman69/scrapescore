...
class Glassdoor(Scraper):
    ...
    def _get_csrf_token(self):
        ...
        """
        res = self.session.get(f"{self.base_url}/Job/computer-science-jobs.htm", timeout=15)
        pattern = r'"token":\s*"([^"]+)"'
    ...
    def _process_job(self, job_data):
        ...
        logger.info(f"Fetched details for job: {job_id}, Title: {title}, Description length: {len(description)} chars")
        return JobPost(
            id=f"gd-{job_id}",
            title=title,
