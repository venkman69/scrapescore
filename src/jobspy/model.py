...
class Country(Enum):
    ...
    def from_string(cls, country_str: str):
        ...
        for country in cls:
            # Check names in the first element of the tuple
            country_names = country.value[0].split(",")
            if country_str in country_names:
                return country
            # Also check the ISO code in the second element (handling : separator)
            iso_code = (
                country.value[1].split(":")[1]
                if ":" in country.value[1]
                else country.value[1]
            )
            if country_str == iso_code.lower():
                return country
...
class JobPost(BaseModel):
    ...
    security_clearance: int | None = None
...
class ScraperInput(BaseModel):
    ...
    # Site-specific configuration for multi-site scrapers (OracleCloud, WorkDay, EightFold)
    # For single-site scrapers (LinkedIn, Indeed), this can be None
    # For multi-site scrapers, this specifies which site to scrape (e.g., "nfcu", "capone", "microsoft")
...
class Scraper(ABC):
    def __init__(
        ...
        self.conn = self._get_job_finder_db()

    def _get_job_finder_db(self):
        if self.db_path is None:
            return None
        if self.db_path.exists():
            return sqlite3.connect(self.db_path)
        logger.warning(f"Database file {self.db_path} does not exist; DB features disabled.")
        return None

    def _check_if_job_exists(self, job_url: str) -> bool:
        if self.conn is None:
            return False
        cursor = self.conn.cursor()
        cursor.execute("SELECT * FROM job_details WHERE job_url = ?", (job_url,))
    ...
    def log_scraping_activity(
        ...
        """Log scraping activity to database (JIRA-095).

        This keeps jobspy loosely coupled from the application - all database
        interactions are managed within the Scraper class, similar to
        _check_if_job_exists(). If the database schema changes, updates are
        isolated to this class.
        ...
        Args:
            ...
            error_message: Error details if the scrape failed
        ...
        cursor.execute('''
            ...
            (run_id, scraper, site, search_term, jobs_found, scrape_duration_seconds, success, error_message)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        ''', (run_id, self.site.value, site, search_term, jobs_found, scrape_duration_seconds, success, error_message))
        self.conn.commit()
